...
def validate(a, b, c, d, e, f, g, Dec, Num):
    ...
    m = SewerSystem(swds_no_meas_area=a, mss_no_meas_area=b, prev_stor_swds_t0=0, prev_so_swds_t0=0, prev_stor_mss_t0=0,
                    prev_so_mss_t0=0, q_swds_ow_cap=c, q_mss_out_cap=d, q_mss_ow_cap=e, stor_swds_cap=f, stor_mss_cap=g)
    data_py = [{'sum_r_swds': 0, 'r_meas_swds': 0, 'sum_r_mss': 0, 'r_meas_mss': 0, 'q_swds_ow': 0, 'q_mss_out': 0,
                'q_mss_ow': 0, 'so_swds_ow': 0, 'so_mss_ow': 0, 'stor_swds': 0, 'stor_mss': 0}]
    ...
    pr_no_meas_area, cp_no_meas_area, op_no_meas_area, ow_no_meas_area, tot_meas_area = 1560, 803.39, 481.61, 300, 0

    t = 1
    while t <= iters - 1:
        data_py.append(m.sol(pr_no_meas_area, cp_no_meas_area, op_no_meas_area, r_pr_swds[t], r_cp_swds[t], r_op_swds[t], r_pr_mss[t]
...
class TestOpenPaved(unittest.TestCase):
    ...
    def test_sol(self):
        ...
        self.assertAlmostEqual(self.ss_1.sol(1560, 803.390641, 481.6093594, 6.766687196, 6.766687196, 6.725020529,
                                             0, 0, 0, 0, 0, 300, 0)['sum_r_swds'], 6.7596337490, places=8)
        ...
        self.assertAlmostEqual(self.ss_1.sol(1560, 803.390641, 481.6093594, 0.670687196, 0.670687196, 0.629020529, 0, 0,
                                             0, 0, 0, 300, 0)['q_swds_ow'], 0.663633749, places=8)
    ...
    def test_integration(self):
        ...
        for n in validate(2845, 0, 55.1, 26.3, 48.1, 2, 9, Dec=7, Num=0):  # default
            self.assertIsNone(n)
        for n in validate(2845, 0, 551, 26.3, 48.1, 2, 9, Dec=7, Num=1):  # q_swds_ow_cap = 551
            self.assertIsNone(n)
        for n in validate(2845, 0, 0, 26.3, 48.1, 2, 9, Dec=7, Num=2):  # q_swds_ow_cap = 0
            self.assertIsNone(n)
        for n in validate(2845, 0, 55.1, 263, 48.1, 2, 9, Dec=7, Num=3):  # q_mss_out_cap = 263
            self.assertIsNone(n)
        for n in validate(2845, 0, 55.1, 0, 48.1, 2, 9, Dec=7, Num=4):  # q_mss_out_cap = 0
            self.assertIsNone(n)
        for n in validate(2845, 0, 55.1, 26.3, 481, 2, 9, Dec=7, Num=5):  # q_mss_ow_cap = 481
            self.assertIsNone(n)
        for n in validate(2845, 0, 55.1, 26.3, 0, 2, 9, Dec=7, Num=6):  # q_mss_ow_cap = 0
            self.assertIsNone(n)
        for n in validate(2845, 0, 37.1, 26.3, 48.1, 20, 9, Dec=7, Num=7):  # q_swds_ow_cap = 37.1, stor_swds_cap = 20
            self.assertIsNone(n)
        for n in validate(2845, 0, 57.1, 26.3, 48.1, 0, 9, Dec=7, Num=8):  # q_swds_ow_cap = 57.1, stor_swds_cap = 0
            self.assertIsNone(n)
        for n in validate(2845, 0, 55.1, 26.3, 48.1, 2, 90, Dec=7, Num=9):  # stor_mss_cap = 90
            self.assertIsNone(n)
        for n in validate(2845, 0, 55.1, 26.3, 48.1, 2, 0, Dec=7, Num=10):  # stor_mss_cap = 0
            self.assertIsNone(n)
