...
def validate(a, b, c, d, e, f, g, h, i, j, Dec, Num):
    ...
    m = Groundwater(init_gwl_t0=a, gw_no_meas_area=b, gw_meas_area=c, seep_def=d, w=e, vc=f, h_deepgw=g,
                    flux=h, soiltype=i, croptype=j)
    data_py = [{'sum_p_gw': 0, 'r_meas_gw': 0, 'gwl_up_1': 0, 'gwl_low_1': 0, 'sc_gw': 0,
                'h_gw': 0, 's_gw_out': 0, 'd_gw_ow': 0, 'gwl': a, 'gwl_sl': 0}]

    p_uz_gw = pd.read_csv(path / "integration_test" / "p_uz_gw.csv")['p_uz_gw_' + str(Num)]
    p_op_gw = pd.read_csv(path / "integration_test" / "p_op_gw.csv")['p_op_gw_' + str(Num)]
    owl = pd.read_csv(path / "integration_test" / "owl.csv")['owl_' + str(Num)]
    uz_no_meas_area = 6855

    t = 1
    while t <= iters - 1:
        data_py.append(m.sol(p_uz_gw[t], uz_no_meas_area, p_op_gw[t], op_no_meas_area=300, tot_meas_area=c,
                             meas_gw=meas_gw[t], prev_owl=owl[t-1], delta_t=1/24))
...
class TestOpenPaved(unittest.TestCase):
    ...
    def test_sol(self):
        ...
        self.assertAlmostEqual(self.gw_1.sol(0.8231929258, 6855, 0, 481.6093594, 0, 0, 1.5, 1 / 24)['sc_gw'],
                               0.21189210852, places=8)

        # time level t = 12/17/1989 15:00
        self.gw_1.prev_gwl = 1.5589210852  # update state
        self.gw_1.prev_gwl_sl = 0
        self.assertAlmostEqual(self.gw_1.sol(0.8231929258, 6855, 0, 481.6093594, 0, 0, 1.5, 1/24)['sum_p_gw'],
                               0.6932417084, places=8)

        # time level t = 12/17/1989 16:00
        self.gw_1.prev_gwl = 1.5588655980  # update state
        self.gw_1.prev_gwl_sl = 0
        self.assertAlmostEqual(self.gw_1.sol(0.3826862970, 6855, 0.041666667, 481.6093594, 0, 0, 1.5, 1/24)['gwl'],
                               1.5588826117, places=9)

    def test_integration(self):
        """
         runs integration tests (validate with excel using different coefficient sets for all time steps)
         """
        for n in validate(1.5, 8140, 0, 0, 100, 20000, 21.5, 1, 2, 1, Dec=3, Num=0):  # default
            self.assertIsNone(n)
        for n in validate(1.5, 8140, 0, 0, 100, 20000, 21.5, 1, 3, 1, Dec=3, Num=1):  # soiltype = 3 croptype = 1
            self.assertIsNone(n)
        for n in validate(1.5, 8140, 0, 0, 100, 20000, 21.5, 1, 7, 1, Dec=3, Num=2):  # soiltype = 7 croptype = 1
            self.assertIsNone(n)
        for n in validate(1.5, 8140, 0, 1, 100, 20000, 21.5, 1, 2, 1, Dec=3, Num=3):  # seep_def = 1
            self.assertIsNone(n)
        for n in validate(1.5, 8140, 0, 0, 10000, 20000, 21.5, 1, 2, 1, Dec=3, Num=4):  # w = 10000
            self.assertIsNone(n)
        for n in validate(1.5, 8140, 0, 0, 0.1, 20000, 21.5, 1, 2, 1, Dec=3, Num=5):  # w = 0.1 (cannot be 0 otherwise DIV0 ERROR)
            self.assertIsNone(n)
        for n in validate(1.5, 8140, 0, 0, 100, 2000000, 21.5, 1, 2, 1, Dec=3, Num=6):  # vc = 2000000
            self.assertIsNone(n)
        for n in validate(1.5, 8140, 0, 0, 100, 0, 21.5, 1, 2, 1, Dec=3, Num=7):  # vc = 0
            self.assertIsNone(n)
        for n in validate(1.5, 8140, 0, 1, 100, 20000, 215, 1, 2, 1, Dec=3, Num=8):  # h_deepgw = 215, seep_def = 1
            self.assertIsNone(n)
        for n in validate(1.5, 8140, 0, 1, 100, 20000, 0, 1, 2, 1, Dec=3, Num=9):  # h_deepgw = 0, seep_def = 1
            self.assertIsNone(n)
        for n in validate(1.5, 0, 8140, 0, 100, 20000, 21.5, 1, 2, 1, Dec=3, Num=10):  # gw_no_meas_area = 0
            self.assertIsNone(n)
