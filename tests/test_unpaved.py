...
def validate(a, b, c, d, e, f, g, h, Dec, Num):
    ...
    m = Unpaved(fin_stor_up_t0=0, up_no_meas_area=a, up_meas_area=b, up_meas_inflow_area=c,
                    infilcap_up=d, intstorcap_up=e, soiltype=f, croptype=g)
    data_py = [{'sum_r_up': 0, 'init_stor_up': 0, 'act_infilcap_up': 0, 'tfac_up': 0, 'e_atm_up': 0, 'i_up_uz': 0,
                'fin_stor_up': 0, 'r_up_meas': 0, 'r_up_ow': 0}]
    t = 1
    while t <= iters - 1:
        data_py.append(m.sol(P_atm[t], E_pot_OW[t], r_pr_up[t], r_cp_up[t], r_op_up[t], theta_rz[t-1],
                             pr_no_meas_area=1560, cp_no_meas_area=803.39, op_no_meas_area=481.61, ow_no_meas_area=h, delta_t=1/24))
        t += 1
...
class TestOpenPaved(unittest.TestCase):
    ...
    def test_sol(self):
        ...
        self.assertAlmostEqual(self.up_1.sol(0, 0.346642066, 0, 0, 0, 175.7714344, 1560, 803.3906406, 481.6093594, 300,
                                             1/24)['act_infilcap_up'], 2)

        # time level t = 8/1/1988 14:00
        self.up_1.prev_fin_stor_up = 0  # update state
        self.assertAlmostEqual(self.up_1.sol(4.826, 0.346642066, 0, 0, 0, 175.5165721, 1560, 803.3906406, 481.6093594,
                                             300, 1/24)['init_stor_up'], 4.826)
    ...
    def test_integration(self):
        ...
        for n in validate(6855, 0, 0, 48, 0, 2, 1, 300, Dec=8, Num=1):  # intstorcap_up = 0
            self.assertIsNone(n)
        for n in validate(6855, 0, 0, 48, 2000, 2, 1, 300, Dec=6, Num=2):  # intstorcap_up = 2000
            self.assertIsNone(n)
        for n in validate(6855, 0, 0, 0, 20, 2, 1, 300, Dec=8, Num=3):  # infilcap_up = 0
            self.assertIsNone(n)
        for n in validate(6855, 0, 0, 4800, 20, 2, 1, 300, Dec=6, Num=4):  # infilcap_up = 4800
            self.assertIsNone(n)
        for n in validate(6855, 0, 0, 48, 20, 3, 1, 300, Dec=8, Num=5):  # soiltype = 3, croptype = 1
            self.assertIsNone(n)
        for n in validate(6855, 0, 0, 48, 20, 4, 1, 300, Dec=8, Num=6):  # soiltype = 4, croptype = 1
            self.assertIsNone(n)
        for n in validate(6855, 0, 0, 48, 20, 2, 1, 300, Dec=6, Num=7):  # discfrac = 0.37 for all three paved areas.
            self.assertIsNone(n)
        for n in validate(0, 6855, 0, 48, 20, 2, 1, 300, Dec=8, Num=8):  # up_no_meas_area = 0
            self.assertIsNone(n)
