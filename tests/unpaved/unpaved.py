...
if __name__ == '__main__':
    ...
    r_pr_up = pd.read_csv(indir / 'r_up.csv')['r_pr_up_0']
    r_cp_up = pd.read_csv(indir / 'r_up.csv')['r_cp_up_0']
    r_op_up = pd.read_csv(indir / 'r_up.csv')['r_op_up_0']
    # b. results of theta_rz from unsaturated zone module.
    theta_rz = pd.read_csv(indir / 'theta_rz.csv')['theta_rz_0']
    ...
    m = Unpaved(fin_stor_up_t0, up_no_meas_area, up_meas_area, up_meas_inflow_area, infilcap_up=48, intstorcap_up=20,
    ...
    def validate(a, b, c, d, e, A1, A2, A3):
        ...
        m = Unpaved(fin_stor_up_t0, up_no_meas_area=A1, up_meas_area=A2, up_meas_inflow_area=A3,
                    infilcap_up=a, intstorcap_up=b, soiltype=c, croptype=d)
        ...
        filename = 'UP_extended_test_pysol' + str(e) + '.csv'
        np.savetxt('pysol/' + filename,
               np.c_[Sum_r_up, Init_stor_up, Act_infilcap_up, Tfac_up, E_atm_up, I_up_uz, Fin_stor_up, R_up_meas, R_up_ow],
               fmt="%.8f", delimiter=',',
               header='Sum_r_up, Init_stor_up, Act_infilcap_up, Tfac_up, E_atm_up, '
    ...
    r_up = pd.read_csv(indir / 'r_up.csv')
    theta_rz_all = pd.read_csv(indir / 'theta_rz.csv')
    ...
    r_op_up = r_up['r_op_up_'+str(e)]
    theta_rz = theta_rz_all['theta_rz_'+str(e)]
    validate(48, 2000, 2, 1, e, 6855, 0, 0)
    ...
    r_op_up = r_up['r_op_up_'+str(e)]
    theta_rz = theta_rz_all['theta_rz_'+str(e)]
    validate(4800, 20, 2, 1, e, 6855, 0, 0)
