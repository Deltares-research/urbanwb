...
def validate(a, b, c, d, e, f, Dec, Num):
    ...
    m = ClosedPaved(init_intstor_cp_t0=0, cp_no_meas_area=a, cp_meas_area=b, cp_meas_inflow_area=c
                    , intstorcap_cp=d, stormfrac_cp=e, discfrac_cp=f)
    data_py = [{'int_cp': 0, 'e_atm_cp': 0, 'intstor_cp': 0, 'r_cp_meas': 0,
                'r_cp_swds': 0, 'r_cp_mss': 0, 'r_cp_up': 0}]
    t = 1
    while t<= iters - 1:
...
class TestClosedPaved(unittest.TestCase):
    ...
    def test_sol(self):
        ...
        self.assertAlmostEqual(self.cp_1.sol(8.89, 0.111267176)['int_cp'], 1.6, places=8)

        # time level t = 4/22/1990 20:00
        self.cp_1.init_intstor_cp = 1.488732824  # update state
        self.assertAlmostEqual(self.cp_1.sol(0.254, 0)['r_cp_swds'], 0.142732824, places=8)
