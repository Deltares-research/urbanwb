from trial_selector import soil_selector, et_selector
from trial_groundwater import Groundwater
import pandas as pd
import numpy as np
import time
...
p_uz_gw = pd.read_csv(path + 'p_uz_gw.csv')['p_uz_gw_0']
p_op_gw = pd.read_csv(path + 'p_op_gw.csv')['p_op_gw_0']
# b. results of open water level.
owl = pd.read_csv(path + 'owl.csv')['owl_0']
...
filename = 'GW_new_test_pysol.csv'
np.savetxt(filename, np.c_[Sum_p_gw, R_meas_gw, Sc_gw, H_gw, S_gw_out, D_gw_ow, Gwl, Gwl_sl],
           fmt="%.8f", delimiter=',',
...
data_py = pd.read_csv(filename)
data_ex = pd.read_csv('GW_general_test_exsol.csv')
A = np.zeros((43825, 8))
for c in range(8):
...
def validate(a, b, c, d, e, f, g, h, i, A1, A2):
    ...
    Gwl_up = [0]
    Gwl_low = [0]
    Sc_gw = [soil_selector(f, g)[0]['stor_coef']]
    H_gw = [0]
    S_gw_out = [0]
    D_gw_ow = [0]
    init_gwl_t0 = h
...
P_uz_gw = pd.read_csv(path + 'p_uz_gw.csv')
P_op_gw = pd.read_csv(path + 'p_op_gw.csv')
Owl = pd.read_csv(path + 'owl.csv')
