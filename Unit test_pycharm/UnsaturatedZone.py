...
# Load csv file
path = "C:/Users/ZWX/PycharmProjects/UWM/Unit_test/UnsaturatedZone/"
...
class UnsaturatedZone:
    ...
    def sol(self, i_up_uz, meas_uz, tot_meas_area, e_ref, prev_gwl, delta_t=1 / 24):
        ...
        if self.uz_no_meas_area == 0:
            ...
            if prev_gwl < 10:
                theta_eq_uz = soil_selector(self.soiltype, self.croptype, gwl_low_uz)['moist_cont_eq_rz[mm]'].values + (
                            gwl_low_uz - prev_gwl) / (gwl_low_uz - gwl_up_uz) * (
                                          soil_selector(self.soiltype, self.croptype, gwl_up_uz)[
                                              'moist_cont_eq_rz[mm]'].values -
                                          soil_selector(self.soiltype, self.croptype, gwl_low_uz)[
                                              'moist_cont_eq_rz[mm]'].values)
                capris_max_uz = soil_selector(self.soiltype, self.croptype, gwl_low_uz)['capris_max[mm/d]'].values + (
                            gwl_low_uz - prev_gwl) / (gwl_low_uz - gwl_up_uz) * (
                                            soil_selector(self.soiltype, self.croptype, gwl_up_uz)[
                ...
                theta_eq_uz = soil_selector(self.soiltype, self.croptype, 10)['moist_cont_eq_rz[mm]'].values
                capris_max_uz = soil_selector(self.soiltype, self.croptype, 10)['capris_max[mm/d]'].values
...
def validate(a, b, c, e, A1, A2):
    ...
    Capris_max_uz = [0]
    P_uz_gw = [0]
    theta_uz_t0 = soil_selector(a, b, c)['moist_cont_eq_rz[mm]'].values  # c is initial gwl [m].
    Theta_uz = [theta_uz_t0]
...
i_up = pd.read_csv(path + 'inputdata/i_up_uz.csv')
gwl_all = pd.read_csv(path + 'inputdata/gwl.csv')
...
i_up_uz = i_up['i_up_uz_'+str(e)]
gwl = gwl_all['gwl_'+str(e)]
validate(2, 1, 3, 3, 6855, 0)

# 2.4 Set 4: initial gwl = 0
print("4")
e = 4
i_up_uz = i_up['i_up_uz_'+str(e)]
gwl = gwl_all['gwl_'+str(e)]
